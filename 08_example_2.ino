...
#define BRIGHTNESS_COEFFICIENT 2.55
...
// initialize USS related variables
  ...
  dist_raw = 0.0;
  last_dist_raw = 0.0;// raw distance output from USS (unit: mm)
...
// turn on the LED if the distance is between dist_min and dist_max
    ...
    distance = abs(200- dist_raw);
    
    brightness = distance * BRIGHTNESS_COEFFICIENT;
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading < dist_min || reading > dist_max) reading = last_dist_raw;
  return reading; // return 0 when out of range
